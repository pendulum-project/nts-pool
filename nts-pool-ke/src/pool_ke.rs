...
mod tests {
    ...
    async fn test_keyexchange_keepalive() {
        ...
        let manager = TestManager::new(
            "a.test".into(),
            vec![
                0x80, 1, 0, 2, 0, 0, 0x80, 4, 0, 2, 0, 0, 0, 5, 0, 2, 1, 2, 0, 5, 0, 2, 3, 4, 0, 8,
                0, 0, 0x80, 0, 0, 0,
            ],
            &[0],
            &[AlgorithmDescription { id: 0, keysize: 16 }],
    ...
    async fn test_keyexchange_get_uuid() {
        ...
        conn.write_all(&[
            0x80, 1, 0, 2, 0, 0, 0x80, 4, 0, 2, 0, 0, 0, 14, 0, 4, b't', b'e', b's', b't', 0xCF, 1,
            0, 4, b'u', b'u', b'i', b'd', 0x80, 0, 0, 0,
        ])
        .await
        .unwrap();
        let mut buf = [0u8; MAX_MESSAGE_SIZE as _];
    ...
    async fn test_keyexchange_get_uuid_authfail() {
        ...
        conn.write_all(&[
            0x80, 1, 0, 2, 0, 0, 0x80, 4, 0, 2, 0, 0, 0, 14, 0, 4, b'n', b'o', b'n', b'e', 0xCF, 1,
            0, 4, b'u', b'u', b'i', b'd', 0x80, 0, 0, 0,
        ])
        .await
        .unwrap();
