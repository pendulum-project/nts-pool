...
const USAGE_MSG: &str = "\
usage: nts-pool-dns [-c PATH] [-l LOG_LEVEL]
       nts-pool-dns -h
       nts-pool-dns -v";

const DESCRIPTOR: &str = "ntp-pool-dns - A DNS server for nts pools";
...
pub(crate) struct NtsPoolDnsOptions {
        ...
        while let Some(arg) = arg_iter.next() {
            match arg.as_str() {
                ...
                long_arg if long_arg.starts_with("--") => {
                    ...
                    let invalid = Err(eyre!("invalid option: '{long_arg}'"));
                    ...
                    if let Some((key, value)) = long_arg.split_once('=') {
                        ...
                        if let Some(next) = arg_iter.next() {
                            ...
                            Err(eyre!("'{}' expects an argument", &long_arg))?;
                        }
                    } else {
                    ...
                    for (n, char) in short_arg.trim_start_matches('-').chars().enumerate() {
                        ...
                        if takes_argument_short.contains(&char) {
                            ...
                            if !rest.is_empty() {
                                ...
                                Err(eyre!("'-{char}' expects an argument"))?;
                            }
                            break;
                        } else {
                            processed.push(CliArg::Flag(flag));
...
impl NtsPoolDnsOptions {
    ...
    pub fn try_parse_from<I, T>(iter: I) -> Result<Self, eyre::Report>
    where
        I: IntoIterator<Item = T>,
        T: AsRef<str> + Clone,
    {
        let mut options = NtsPoolDnsOptions::default();
        ...
        for arg in arg_iter {
            match arg {
                CliArg::Flag(flag) => match flag.as_str() {
                    ...
                    option => {
                        Err(eyre!("invalid option provided: {option}"))?;
                    }
                },
                CliArg::Argument(option, value) => match option.as_str() {
                    ...
                    "-l" | "--log-level" => match LogLevel::from_str(&value) {
                        ...
                        Err(_) => return Err(eyre!("invalid log level")),
                    },
                    option => {
                        Err(eyre!("invalid option provided: {option}"))?;
                    }
    ...
    fn resolve_action(&mut self) {
        if self.help {
            self.action = NtsPoolDnsAction::Help;
        } else if self.version {
            self.action = NtsPoolDnsAction::Version;
        } else {
            self.action = NtsPoolDnsAction::Run;
        }
    }
}
